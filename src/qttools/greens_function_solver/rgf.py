...
class RGF(GFSolver):
    ...
    def __init__(self, max_batch_size: int = 100, mask: str | None = None) -> None:
    ...
    def selected_inv(
        ...
        for b in range(len(batches_sizes)):
            ...
            for i in range(a.num_blocks - 1):
                ...
                temp_mult = complex_gemm_to_real_with_mask(a_.blocks[j, i], x_diag_blocks[i], mask=self.mask)
                temp_result = complex_gemm_to_real_with_mask(temp_mult, a_.blocks[i, j], mask=self.mask)
            ...
            for i in range(a.num_blocks - 2, -1, -1):
                ...
                temp_mult1 = complex_gemm_to_real_with_mask(x_jj, a_.blocks[j, i], mask=self.mask)
                x_ji = -complex_gemm_to_real_with_mask(temp_mult1, x_ii, mask=self.mask)
                x_.blocks[j, i] = x_ji
                temp_mult2 = complex_gemm_to_real_with_mask(x_ii, a_ij, mask=self.mask)
                x_.blocks[i, j] = -complex_gemm_to_real_with_mask(temp_mult2, x_jj, mask=self.mask)

                # NOTE: Cursed Python multiple assignment syntax.
                temp_mult3 = complex_gemm_to_real_with_mask(x_ii, a_ij, mask=self.mask)
                temp_result3 = complex_gemm_to_real_with_mask(temp_mult3, x_ji, mask=self.mask)
    ...
    def selected_solve(
        ...
        for i in range(len(batches_sizes)):
            ...
            temp_mult_l0 = complex_gemm_to_real_with_mask(xr_jj, sl_jj, mask=self.mask)
            xl_diag_blocks[0] = complex_gemm_to_real_with_mask(temp_mult_l0, xr_jj_dagger, mask=self.mask)
            temp_mult_g0 = complex_gemm_to_real_with_mask(xr_jj, sg_jj, mask=self.mask)
            xg_diag_blocks[0] = complex_gemm_to_real_with_mask(temp_mult_g0, xr_jj_dagger, mask=self.mask)
            ...
            for i in range(a.num_blocks - 1):
                ...
                a_ji_xr_ii = complex_gemm_to_real_with_mask(a_ji, xr_ii, mask=self.mask)

                temp_inv_arg = complex_gemm_to_real_with_mask(a_ji_xr_ii, a_.blocks[i, j], mask=self.mask)
                xr_jj = inv(a_jj - temp_inv_arg)
                xr_jj_dagger = xr_jj.conj().swapaxes(-2, -1)
                xr_diag_blocks[j] = xr_jj

                a_ji_xr_ii_sx_ij = complex_gemm_to_real_with_mask(a_ji_xr_ii, sigma_lesser_.blocks[i, j], mask=self.mask)

                temp_xl_inner1 = complex_gemm_to_real_with_mask(a_ji, xl_diag_blocks[i], mask=self.mask)
                temp_xl_inner2 = complex_gemm_to_real_with_mask(temp_xl_inner1, a_ji_dagger, mask=self.mask)
                temp_xl_sum = (
                    sl_jj
                    + temp_xl_inner2
                ...
                temp_xl_mult1 = complex_gemm_to_real_with_mask(xr_jj, temp_xl_sum, mask=self.mask)
                xl_diag_blocks[j] = complex_gemm_to_real_with_mask(temp_xl_mult1, xr_jj_dagger, mask=self.mask)

                a_ji_xr_ii_sx_ij = complex_gemm_to_real_with_mask(a_ji_xr_ii, sigma_greater_.blocks[i, j], mask=self.mask)

                temp_xg_inner1 = complex_gemm_to_real_with_mask(a_ji, xg_diag_blocks[i], mask=self.mask)
                temp_xg_inner2 = complex_gemm_to_real_with_mask(temp_xg_inner1, a_ji_dagger, mask=self.mask)
                temp_xg_sum = (
                    sg_jj
                    + temp_xg_inner2
                    + a_ji_xr_ii_sx_ij.conj().swapaxes(-2, -1)
                ...
                temp_xg_mult1 = complex_gemm_to_real_with_mask(xr_jj, temp_xg_sum, mask=self.mask)
                xg_diag_blocks[j] = complex_gemm_to_real_with_mask(temp_xg_mult1, xr_jj_dagger, mask=self.mask)
            ...
            for i in range(a.num_blocks - 2, -1, -1):
                ...
                xr_ii_a_ij = complex_gemm_to_real_with_mask(xr_ii, a_ij, mask=self.mask)
                a_ij_dagger_xr_ii_dagger = xr_ii_a_ij.conj().swapaxes(-2, -1)
                xr_jj_a_ji = complex_gemm_to_real_with_mask(xr_jj, a_ji, mask=self.mask)
                a_ji_dagger_xr_jj_dagger = xr_jj_a_ji.conj().swapaxes(-2, -1)
                xr_ii_a_ij_xr_jj = complex_gemm_to_real_with_mask(xr_ii_a_ij, xr_jj, mask=self.mask)
                xr_jj_dagger_a_ij_dagger_xr_ii_dagger = (
                    xr_ii_a_ij_xr_jj.conj().swapaxes(-2, -1)
                )
                xr_ii_a_ij_xr_jj_a_ji = complex_gemm_to_real_with_mask(xr_ii_a_ij, xr_jj_a_ji, mask=self.mask)

                temp_1x_part1 = complex_gemm_to_real_with_mask(xr_ii_a_ij_xr_jj_a_ji, xl_ii, mask=self.mask)
                temp_1x_part2_1 = complex_gemm_to_real_with_mask(xr_ii, sigma_lesser_ij, mask=self.mask)
                temp_1x_part2 = complex_gemm_to_real_with_mask(temp_1x_part2_1, xr_jj_dagger_a_ij_dagger_xr_ii_dagger, mask=self.mask)
                temp_1x = temp_1x_part1 - temp_1x_part2
                temp_1x -= temp_1x.conj().swapaxes(-2, -1)
                temp_2x = complex_gemm_to_real_with_mask(xr_ii_a_ij, xl_jj, mask=self.mask)

                xl_ij_part1 = complex_gemm_to_real_with_mask(xl_ii, a_ji_dagger_xr_jj_dagger, mask=self.mask)
                xl_ij_part2_1 = complex_gemm_to_real_with_mask(xr_ii, sigma_lesser_ij, mask=self.mask)
                xl_ij_part2 = complex_gemm_to_real_with_mask(xl_ij_part2_1, xr_jj_dagger, mask=self.mask)
                ...
                xl_diag_part = complex_gemm_to_real_with_mask(temp_2x, a_ij_dagger_xr_ii_dagger, mask=self.mask)
                xl_diag_blocks[i] = xl_ii + xl_diag_part + temp_1x
                xl_.blocks[i, i] = 0.5 * (
                    xl_diag_blocks[i] - xl_diag_blocks[i].conj().swapaxes(-2, -1)
                )

                temp_1x_part1_g = complex_gemm_to_real_with_mask(xr_ii_a_ij_xr_jj_a_ji, xg_ii, mask=self.mask)
                temp_1x_part2_1_g = complex_gemm_to_real_with_mask(xr_ii, sigma_greater_ij, mask=self.mask)
                temp_1x_part2_g = complex_gemm_to_real_with_mask(temp_1x_part2_1_g, xr_jj_dagger_a_ij_dagger_xr_ii_dagger, mask=self.mask)
                temp_1x = temp_1x_part1_g - temp_1x_part2_g
                temp_1x -= temp_1x.conj().swapaxes(-2, -1)
                temp_2x = complex_gemm_to_real_with_mask(xr_ii_a_ij, xg_jj, mask=self.mask)

                xg_ij_part1 = complex_gemm_to_real_with_mask(xg_ii, a_ji_dagger_xr_jj_dagger, mask=self.mask)
                xg_ij_part2_1 = complex_gemm_to_real_with_mask(xr_ii, sigma_greater_ij, mask=self.mask)
                xg_ij_part2 = complex_gemm_to_real_with_mask(xg_ij_part2_1, xr_jj_dagger, mask=self.mask)
                ...
                xg_diag_part = complex_gemm_to_real_with_mask(temp_2x, a_ij_dagger_xr_ii_dagger, mask=self.mask)
                ...
                if return_current:
                    a_ji_xr_ii_curr = complex_gemm_to_real_with_mask(a_ji, xr_ii, mask=self.mask)
                    a_ji_xr_ii_sx_ij_curr = complex_gemm_to_real_with_mask(a_ji_xr_ii_curr, sigma_lesser_ij, mask=self.mask)
                    temp_curr_l1 = complex_gemm_to_real_with_mask(a_ji, xl_ii, mask=self.mask)
                    temp_curr_l2 = complex_gemm_to_real_with_mask(temp_curr_l1, a_ji_dagger, mask=self.mask)
                    sigma_lesser_tilde = (
                        temp_curr_l2
                        + a_ji_xr_ii_sx_ij_curr.conj().swapaxes(-2, -1)
                        - a_ji_xr_ii_sx_ij_curr
                    )
                    a_ji_xr_ii_sx_ij_curr = complex_gemm_to_real_with_mask(a_ji_xr_ii_curr, sigma_greater_ij, mask=self.mask)
                    temp_curr_g1 = complex_gemm_to_real_with_mask(a_ji, xg_ii, mask=self.mask)
                    temp_curr_g2 = complex_gemm_to_real_with_mask(temp_curr_g1, a_ji_dagger, mask=self.mask)
                    sigma_greater_tilde = (
                        temp_curr_g2
                        + a_ji_xr_ii_sx_ij_curr.conj().swapaxes(-2, -1)
                        - a_ji_xr_ii_sx_ij_curr
                    )
                    temp_trace1 = complex_gemm_to_real_with_mask(sigma_greater_tilde, xl_diag_blocks[j], mask=self.mask)
                    temp_trace2 = complex_gemm_to_real_with_mask(xg_diag_blocks[j], sigma_lesser_tilde, mask=self.mask)
                ...
                xr_final_mult = complex_gemm_to_real_with_mask(xr_ii_a_ij_xr_jj_a_ji, xr_ii, mask=self.mask)
