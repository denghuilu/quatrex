...
from qttools.datastructures.routines import complex_gemm_to_real_high_precision
...
class RGF(GFSolver):
    ...
    def selected_inv(
        ...
        for b in range(len(batches_sizes)):
            ...
            for i in range(a.num_blocks - 1):
                ...
                temp_mult = complex_gemm_to_real_high_precision(a_.blocks[j, i], x_diag_blocks[i])
                temp_result = complex_gemm_to_real_high_precision(temp_mult, a_.blocks[i, j])
            ...
            for i in range(a.num_blocks - 2, -1, -1):
                ...
                temp_mult1 = complex_gemm_to_real_high_precision(x_jj, a_.blocks[j, i])
                x_ji = -complex_gemm_to_real_high_precision(temp_mult1, x_ii)
                x_.blocks[j, i] = x_ji
                temp_mult2 = complex_gemm_to_real_high_precision(x_ii, a_ij)
                x_.blocks[i, j] = -complex_gemm_to_real_high_precision(temp_mult2, x_jj)

                # NOTE: Cursed Python multiple assignment syntax.
                temp_mult3 = complex_gemm_to_real_high_precision(x_ii, a_ij)
                temp_result3 = complex_gemm_to_real_high_precision(temp_mult3, x_ji)
    ...
    def selected_solve(
        ...
        for i in range(len(batches_sizes)):
            ...
            ).astype(xp.complex64).astype(xp.complex128)
            ...
            temp_mult_l0 = complex_gemm_to_real_high_precision(xr_jj, sl_jj)
            xl_diag_blocks[0] = complex_gemm_to_real_high_precision(temp_mult_l0, xr_jj_dagger)
            temp_mult_g0 = complex_gemm_to_real_high_precision(xr_jj, sg_jj)
            xg_diag_blocks[0] = complex_gemm_to_real_high_precision(temp_mult_g0, xr_jj_dagger)
            ...
            for i in range(a.num_blocks - 1):
                ...
                a_ji = a_.blocks[j, i].astype(xp.complex64).astype(xp.complex128)
                ...
                a_ji_xr_ii = complex_gemm_to_real_high_precision(a_ji, xr_ii)

                temp_inv_arg = complex_gemm_to_real_high_precision(a_ji_xr_ii, a_.blocks[i, j].astype(xp.complex64).astype(xp.complex128))
                xr_jj = inv(a_jj - temp_inv_arg)
                xr_jj_dagger = xr_jj.conj().swapaxes(-2, -1)
                xr_diag_blocks[j] = xr_jj

                a_ji_xr_ii_sx_ij = complex_gemm_to_real_high_precision(a_ji_xr_ii, sigma_lesser_.blocks[i, j].astype(xp.complex64).astype(xp.complex128))

                temp_xl_inner1 = complex_gemm_to_real_high_precision(a_ji, xl_diag_blocks[i])
                temp_xl_inner2 = complex_gemm_to_real_high_precision(temp_xl_inner1, a_ji_dagger)
                temp_xl_sum = (
                    sl_jj
                    + temp_xl_inner2
                ...
                temp_xl_mult1 = complex_gemm_to_real_high_precision(xr_jj, temp_xl_sum)
                xl_diag_blocks[j] = complex_gemm_to_real_high_precision(temp_xl_mult1, xr_jj_dagger)

                a_ji_xr_ii_sx_ij = complex_gemm_to_real_high_precision(a_ji_xr_ii, sigma_greater_.blocks[i, j].astype(xp.complex64).astype(xp.complex128))

                temp_xg_inner1 = complex_gemm_to_real_high_precision(a_ji, xg_diag_blocks[i])
                temp_xg_inner2 = complex_gemm_to_real_high_precision(temp_xg_inner1, a_ji_dagger)
                temp_xg_sum = (
                    sg_jj
                    + temp_xg_inner2
                    + a_ji_xr_ii_sx_ij.conj().swapaxes(-2, -1)
                ...
                temp_xg_mult1 = complex_gemm_to_real_high_precision(xr_jj, temp_xg_sum)
                xg_diag_blocks[j] = complex_gemm_to_real_high_precision(temp_xg_mult1, xr_jj_dagger)
            ...
            if return_retarded:
                xr_.blocks[a.num_blocks - 1, a.num_blocks - 1] = xr_diag_blocks[-1].astype(xp.complex64).astype(xp.complex128)
            ...
            for i in range(a.num_blocks - 2, -1, -1):
                ...
                a_ij = a_.blocks[i, j].astype(xp.complex64).astype(xp.complex128)
                a_ji = a_.blocks[j, i].astype(xp.complex64).astype(xp.complex128)
                xl_ii = xl_diag_blocks[i]
                xl_jj = xl_diag_blocks[j]
                xg_ii = xg_diag_blocks[i]
                xg_jj = xg_diag_blocks[j]
                sigma_lesser_ij = sigma_lesser_.blocks[i, j].astype(xp.complex64).astype(xp.complex128)
                sigma_greater_ij = sigma_greater_.blocks[i, j].astype(xp.complex64).astype(xp.complex128)

                # Precompute the transposes that are used multiple times.
                xr_jj_dagger = xr_jj.conj().swapaxes(-2, -1)

                # Precompute the terms that are used multiple times.
                xr_ii_a_ij = complex_gemm_to_real_high_precision(xr_ii, a_ij)
                a_ij_dagger_xr_ii_dagger = xr_ii_a_ij.conj().swapaxes(-2, -1)
                xr_jj_a_ji = complex_gemm_to_real_high_precision(xr_jj, a_ji)
                a_ji_dagger_xr_jj_dagger = xr_jj_a_ji.conj().swapaxes(-2, -1)
                xr_ii_a_ij_xr_jj = complex_gemm_to_real_high_precision(xr_ii_a_ij, xr_jj)
                xr_jj_dagger_a_ij_dagger_xr_ii_dagger = (
                    xr_ii_a_ij_xr_jj.conj().swapaxes(-2, -1)
                )
                xr_ii_a_ij_xr_jj_a_ji = complex_gemm_to_real_high_precision(xr_ii_a_ij, xr_jj_a_ji)

                temp_1x_part1 = complex_gemm_to_real_high_precision(xr_ii_a_ij_xr_jj_a_ji, xl_ii)
                temp_1x_part2_1 = complex_gemm_to_real_high_precision(xr_ii, sigma_lesser_ij)
                temp_1x_part2 = complex_gemm_to_real_high_precision(temp_1x_part2_1, xr_jj_dagger_a_ij_dagger_xr_ii_dagger)
                temp_1x = temp_1x_part1 - temp_1x_part2
                temp_1x -= temp_1x.conj().swapaxes(-2, -1)
                temp_2x = complex_gemm_to_real_high_precision(xr_ii_a_ij, xl_jj)

                xl_ij_part1 = complex_gemm_to_real_high_precision(xl_ii, a_ji_dagger_xr_jj_dagger)
                xl_ij_part2_1 = complex_gemm_to_real_high_precision(xr_ii, sigma_lesser_ij)
                xl_ij_part2 = complex_gemm_to_real_high_precision(xl_ij_part2_1, xr_jj_dagger)
                xl_ij = -temp_2x - xl_ij_part1 + xl_ij_part2

                xl_.blocks[i, j] = xl_ij.astype(xp.complex64).astype(xp.complex128)
                if not xl_.symmetry:
                    xl_.blocks[j, i] = -xl_ij.conj().swapaxes(-2, -1).astype(xp.complex64).astype(xp.complex128)

                xl_diag_part = complex_gemm_to_real_high_precision(temp_2x, a_ij_dagger_xr_ii_dagger)
                xl_diag_blocks[i] = xl_ii + xl_diag_part + temp_1x
                xl_.blocks[i, i] = 0.5 * (
                    xl_diag_blocks[i] - xl_diag_blocks[i].conj().swapaxes(-2, -1)
                ).astype(xp.complex64).astype(xp.complex128)

                temp_1x_part1_g = complex_gemm_to_real_high_precision(xr_ii_a_ij_xr_jj_a_ji, xg_ii)
                temp_1x_part2_1_g = complex_gemm_to_real_high_precision(xr_ii, sigma_greater_ij)
                temp_1x_part2_g = complex_gemm_to_real_high_precision(temp_1x_part2_1_g, xr_jj_dagger_a_ij_dagger_xr_ii_dagger)
                temp_1x = temp_1x_part1_g - temp_1x_part2_g
                temp_1x -= temp_1x.conj().swapaxes(-2, -1)
                temp_2x = complex_gemm_to_real_high_precision(xr_ii_a_ij, xg_jj)

                xg_ij_part1 = complex_gemm_to_real_high_precision(xg_ii, a_ji_dagger_xr_jj_dagger)
                xg_ij_part2_1 = complex_gemm_to_real_high_precision(xr_ii, sigma_greater_ij)
                xg_ij_part2 = complex_gemm_to_real_high_precision(xg_ij_part2_1, xr_jj_dagger)
                xg_ij = -temp_2x - xg_ij_part1 + xg_ij_part2

                xg_.blocks[i, j] = xg_ij.astype(xp.complex64).astype(xp.complex128)
                if not xg_.symmetry:
                    xg_.blocks[j, i] = -xg_ij.conj().swapaxes(-2, -1).astype(xp.complex64).astype(xp.complex128)

                xg_diag_part = complex_gemm_to_real_high_precision(temp_2x, a_ij_dagger_xr_ii_dagger)
                ...
                if return_current:
                    a_ji_xr_ii_curr = complex_gemm_to_real_high_precision(a_ji, xr_ii)
                    a_ji_xr_ii_sx_ij_curr = complex_gemm_to_real_high_precision(a_ji_xr_ii_curr, sigma_lesser_ij)
                    temp_curr_l1 = complex_gemm_to_real_high_precision(a_ji, xl_ii)
                    temp_curr_l2 = complex_gemm_to_real_high_precision(temp_curr_l1, a_ji_dagger)
                    sigma_lesser_tilde = (
                        temp_curr_l2
                        + a_ji_xr_ii_sx_ij_curr.conj().swapaxes(-2, -1)
                        - a_ji_xr_ii_sx_ij_curr
                    )
                    a_ji_xr_ii_sx_ij_curr = complex_gemm_to_real_high_precision(a_ji_xr_ii_curr, sigma_greater_ij)
                    temp_curr_g1 = complex_gemm_to_real_high_precision(a_ji, xg_ii)
                    temp_curr_g2 = complex_gemm_to_real_high_precision(temp_curr_g1, a_ji_dagger)
                    sigma_greater_tilde = (
                        temp_curr_g2
                        + a_ji_xr_ii_sx_ij_curr.conj().swapaxes(-2, -1)
                        - a_ji_xr_ii_sx_ij_curr
                    )
                    temp_trace1 = complex_gemm_to_real_high_precision(sigma_greater_tilde, xl_diag_blocks[j])
                    temp_trace2 = complex_gemm_to_real_high_precision(xg_diag_blocks[j], sigma_lesser_tilde)
                ...
                xr_final_mult = complex_gemm_to_real_high_precision(xr_ii_a_ij_xr_jj_a_ji, xr_ii)
                xr_diag_blocks[i] = xr_ii + xr_final_mult
                if return_retarded:
                    xr_.blocks[i, i] = xr_diag_blocks[i].astype(xp.complex64).astype(xp.complex128)
